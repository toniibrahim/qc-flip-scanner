...
Search the live listing portals below for active residential listings in the priority areas. Use `web_search` and `web_fetch`. If the routine has Firecrawl, Playwright, or a similar MCP scraping connector, prefer it over raw `web_fetch` for JS-heavy pages like Centris.
